...
class ModelManage extends \Core\Model\Model {
    ...
    public static function addModel() {
        ...
        $data['model_id'] = $addResult = self::db('model')->insert($data);
        if ($addResult === FALSE) {
            return false;
        } else {
            return $data;
    ...
    public static function baseFrom() {
        if (self::p('method') == 'PUT') {
            $data['noset']['model_id'] = self::isP('model_id', '丢失模型ID');

            if (!$model = self::findModel($data['noset']['model_id'])) {
                return self::error('不存在的模型');
            }
        } else {
            $data['model_name'] = ucfirst(strtolower(self::isP('model_name', '请填写模型名称')));
        }

        $data['lang_key'] = self::isP('display_name', '请填写模型显示名称');

        $data['is_search'] = self::isP('is_search', '请选择模型是否允许搜索');

        $data['model_attr'] = self::isP('model_attr', '请选择模型属性');

        $data['status'] = self::isP('status', '请选择模型状态');
    ...
    public static function setInitField($modelId) {
        $setStatus = self::db('field')->insert(array('model_id' => $modelId, 'field_name' => 'status', 'display_name' => '状态', 'field_type' => 'radio', 'field_option' => '{"\u7981\u7528":"0","\u542f\u7528":"1"}', 'field_default' => '1', 'field_required' => '1', 'field_listsort' => '100', 'field_status' => '1'));
        if ($setStatus === false) {
            self::error('设置预设字段失败');
        }

        $setListsort = self::db('field')->insert(array('model_id' => $modelId, 'field_name' => 'listsort', 'display_name' => '排序', 'field_type' => 'text', 'field_listsort' => '98', 'field_status' => '1'));
        if ($setListsort === false) {
            self::error('设置预设字段失败');
        }

        $setCreatetime = self::db('field')->insert(array('model_id' => $modelId, 'field_name' => 'createtime', 'display_name' => '创建时间', 'field_type' => 'date', 'field_listsort' => '99', 'field_status' => '1'));
